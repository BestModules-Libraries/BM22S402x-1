...
void setup()
{
  ...
  /* Single trigger mode */
  // error += pir.writeCommand(0x09, 30);   // Delay time: 30*0.1 = 3 sec
  // error += pir.writeCommand(0x0b, 5);    // Block time:  5*0.2 = 1 sec
  // error += pir.writeCommand(0x05, 0x69); // LVD: 2.7V(default), LVD disable, enable PIR, single trigger, AUTO mode

  /* Continue trigger mode */
  error += pir.writeCommand(0x05, 0x6B); // LVD: 2.7V(default), LVD disable, enable PIR, continue trigger, AUTO mode
